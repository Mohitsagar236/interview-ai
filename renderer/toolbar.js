...
(function () {
  const state = {
    ...
    processor: null,
    mediaStream: null,
    userId: null, // User ID from desktop activation (CRITICAL for chat isolation)
    sessionId: null, // Session ID generated by server (for tracking)
  ...
  function send(message) {
    ...
    if (message && typeof message === "object") {
      ...
      // CRITICAL: Add user identification for session isolation
      if (state.userId) {
        message.user_id = state.userId;
      }
      if (state.sessionId) {
  ...
  // CRITICAL: Fetch user data for chat isolation
  (async () => {
    try {
      if (window.electronAPI && typeof window.electronAPI.desktopGetUser === "function") {
        const userResult = await window.electronAPI.desktopGetUser();
        if (userResult && userResult.ok && userResult.user) {
          state.userId = userResult.user.id || userResult.user.email || userResult.user.username;
          console.log(`[Auth] Logged in user: ${state.userId}`);
        } else {
          console.warn("[Auth] Could not get user data:", userResult?.error);
        }
      }
    } catch (e) {
      console.warn("[Auth] Error fetching user data:", e);
    }
  })();
