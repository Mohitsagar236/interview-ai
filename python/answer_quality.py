...
def postprocess_answer(text: str, seen_tokens_log: Set[str]) -> str:
    ...
    # Keep the opening of the final answer intact. The toolbar streams partial
    # chunks first and then replaces them with this complete formatted response;
    # trimming a "seen" prefix here makes answers start mid-sentence.
    code_blocks = []
...
def compute_confidence(answer_text: str, question_type: str = "general") -> Tuple[float, str]:
    ...
    Scoring rules:
        ...
        - +0.10 if code is present for coding questions
    ...
    # 2. Code for coding questions (+0.10)
    ...
    if has_code:
        ...
        if is_coding_question:
            ...
            logger.debug(f"  +0.10: Code present for coding question")

        else:

            score += 0.05  # Still valuable but less critical

            logger.debug(f"  +0.05: Code present")
