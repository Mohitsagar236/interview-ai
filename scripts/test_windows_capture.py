...
def test_availability():
    ...
    assert is_windows_capture_available(), "Windows capture not available (pywin32 may be missing)"
...
def test_screen_capture():
    ...
    assert result, "Screen capture returned None"
...
def test_window_list():
    ...
    assert windows, "No windows found"
...
def test_window_capture():
    ...
    from io import BytesIO
    windows = get_available_windows()
    assert windows, "No windows available to test"
    ...
    assert result, "Window capture returned None"
