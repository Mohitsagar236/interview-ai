...
def capture_screen_cross_platform(monitor_index: int = 0):
    """Cross-platform screen capture using mss. Works on Mac, Linux, and Windows."""
    if not MSS_AVAILABLE:
        import logging
        logging.getLogger(__name__).warning("mss not installed -- install with: pip install mss")
        return None
    try:
        import sys
        ...
        with _mss() as sct:
            ...
            sshot = sct.grab(monitors[idx])
            img = _Image.frombytes("RGB", sshot.size, sshot.bgra, "raw", "BGRX")
            img_b64, w, h = _encode_image(img)
            import logging
            logging.getLogger(__name__).info(f"[CrossPlatform] Captured monitor {monitor_index}: {w}x{h} ({sys.platform})")
            return {"image": img_b64, "width": w, "height": h, "format": "png"}
    except Exception as e:
        import logging
        logging.getLogger(__name__).error(f"Cross-platform capture failed: {e}")
        return None


def is_cross_platform_capture_available() -> bool:
    """True when mss-based cross-platform capture is usable (Mac/Linux/Windows)."""
